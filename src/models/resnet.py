...
class ResNetSmall(nn.Module):
    """Lightweight ResNet for resource-constrained settings (e.g. ResNet8).
    
    Uses 3 layer groups with narrow channels (16→32→64) instead of
    the standard 4 groups (64→128→256→512), resulting in ~75K parameters.
    """
...
def ResNet8(num_classes=10, num_channels=3):
    """Create ResNet8 model (~75K params, suitable for FL on CPU)."""
    return ResNetSmall(BasicBlock, [1, 1, 1], num_classes, num_channels)
