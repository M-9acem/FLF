...
Partitions can be pre-generated once and reused across experiments via `generate_partition.py` and `--partition_file`.

### Reproducibility Utilities

Three canonical artefacts ensure all experiments start from identical conditions:

| Artefact | Generated by | CLI arg | Default auto-save path |
|---|---|---|---|
| Data partition | `generate_partition.py` | `--partition_file` | `data_partition/<dataset>_N<n>_<type>_a<alpha>.pkl` |
| Network topology | `shared_topology.pkl` (committed) | `--topology_file` | `shared_topology.pkl` |
| Initial weights | `generate_init_weights.py` | `--init_weights` | `init_weights/<model>_w0.pt` |

When a canonical file is supplied, `main.py` loads it instead of regenerating, guaranteeing every run of every mixing method sees the same starting point.

### Gossip Step Control

Two mutually exclusive modes control how many gossip iterations happen each round:

- **Fixed steps** (`--gossip_steps N`): same number of gossip steps every round (default: `1`)
- **Gossip schedule** (`--gossip_schedule "steps:from_round,..."`)`: decays over training

The schedule format is a comma-separated list of `steps:from_round` entries, applied in ascending round order. Example:

```
--gossip_schedule "5:0,3:100,1:200"
```

Rounds 0–99 → 5 gossip steps, rounds 100–199 → 3 steps, rounds ≥200 → 1 step.
...
- Pre-gossip and post-gossip model evaluation (decentralized mode)
...
`run_all_mixing_methods.py` runs all four mixing methods sequentially under **identical** conditions (same topology, same initial weights, same data partition). Configurable at the top of the file:

```python
NUM_CLIENTS     = 40
ROUNDS          = 200
EPOCHS          = 2
GOSSIP_STEPS    = 1       # used when GOSSIP_SCHEDULE is None
GOSSIP_SCHEDULE = None    # e.g. "5:0,3:100,1:200"
DATASET         = "cifar10"
```
...
│   │   ├── p2p_client.py
│   │   ├── p2p_runner.py        # Gossip coordinator (supports gossip_schedule)
│   │   └── topology.py
│   ├── models/
│   │   ├── lenet5.py
│   │   ├── simple_cnn.py
...
- `--intra_cluster_communication`: If set, all nodes in a cluster are fully interconnected. If not set (default), each cluster uses a star topology where only the edge node bridges to the other cluster.
...
- `--topology_file`: Path to a pre-saved topology `.pkl` file. When supplied, topology generation is skipped and the file is loaded directly.
...
- `--gossip_steps`: Fixed number of gossip iterations per round (default: `1`)
- `--gossip_schedule`: Variable gossip steps that decay over training. Format: `"steps:from_round,steps:from_round,..."`. Overrides `--gossip_steps` when set. Example: `"5:0,3:100,1:200"` (5 steps for rounds 0–99, 3 for 100–199, 1 afterwards).

#### Reproducibility Parameters

- `--init_weights`: Path to a saved model state dict (`.pt`). All clients are initialised from this instead of random weights.
- `--partition_file`: Path to a pre-generated partition `.pkl` file (produced by `generate_partition.py`). Skips re-partitioning and ensures all experiments use the same data split.
...
#### 6. Decentralized with Fixed Gossip Steps

```bash
python main.py --type decentralized --num_clients 10 --rounds 30 --gossip_steps 3
```

#### 7. Decentralized with Gossip Schedule (decay over training)

```bash
python main.py --type decentralized --num_clients 40 --rounds 200 --gossip_schedule "5:0,3:100,1:200"
```

#### 8. Reproducible Run Using Canonical Artefacts

```bash
# Pre-generate canonical artefacts once:
python generate_partition.py
python generate_init_weights.py

# Every subsequent run is identical regardless of OS/seed:
python main.py --type decentralized --num_clients 40 --rounds 200 \
  --partition_file data_partition/cifar10_N40_dirichlet_a0.5.pkl \
  --topology_file shared_topology.pkl \
  --init_weights init_weights/resnet8_w0.pt \
  --model resnet8 --dataset cifar10
```

#### 9. Compare All Mixing Methods (batch runner)

```bash
python run_all_mixing_methods.py
```

Runs all four mixing methods sequentially, each using the same topology, initial weights, and data partition. Results are saved to separate timestamped subdirectories under `logs/`.
...
- **gradient_tracking.csv**: Gradient norms and variance (pre- and post-gossip)
...
- **partition_stats.csv**: Data partition statistics (samples per client, class distribution)
...
- **topology_info.txt**: Topology details (nodes, edges, degrees)
